...
int validate_node_id(cfg_t* cfg, cfg_opt_t* opt) {
    ...
    for (auto idx = 0; idx < ids; ++idx) {
        ...
        if (id < 0 || id > ExtH9Frame::H9FRAME_SOURCE_ID_MAX_VALUE) {
            cfg_error(cfg, "option '%s' in section '%s' must have a value between 0-%d", opt->name, cfg->name, ExtH9Frame::H9FRAME_SOURCE_ID_MAX_VALUE);
            return -1;
        }
    }
...
int validate_node_type(cfg_t* cfg, cfg_opt_t* opt) {
    ...
    if (type < 1 || type > 0xffff) {
        cfg_error(cfg, "option '%s' in section '%s' must have a value between 1-%d", opt->name, cfg->name, ExtH9Frame::H9FRAME_SOURCE_ID_MAX_VALUE);
        return -1;
    }
