...
static int H9Frame_init(PyH9frame* self, PyObject* args, PyObject* kwds) {
    static const char* kwlist[] = {"type", "source_id", "broadcast_group", "destination_id", "seqnum", "dlc", "data", NULL};

    std::uint8_t type = 0; // RES1 = 0
    std::uint8_t source_id = 0;
    std::uint16_t broadcast_group = 0;
    ...
    if (type > ExtH9Frame::H9FRAME_TYPE_MAX_VALUE) {
        PyErr_SetString(PyExc_OverflowError, ("maximum allowed value is " + std::to_string(ExtH9Frame::H9FRAME_TYPE_MAX_VALUE)).c_str());
        return -1;
    }
    self->frame.type = (uint8_t)type;

    if (seqnum > ExtH9Frame::H9FRAME_SEQNUM_MAX_VALUE) {
        PyErr_SetString(PyExc_OverflowError, ("maximum allowed value is " + std::to_string(ExtH9Frame::H9FRAME_SEQNUM_MAX_VALUE)).c_str());
        return -1;
    }
    self->frame.unicast.seqnum = seqnum;

    if (destination_id > ExtH9Frame::H9FRAME_DESTINATION_ID_MAX_VALUE) {
        PyErr_SetString(PyExc_OverflowError, ("maximum allowed value is " + std::to_string(ExtH9Frame::H9FRAME_DESTINATION_ID_MAX_VALUE)).c_str());
        return -1;
    }
    self->frame.unicast.destination_id = destination_id;

    if (source_id > ExtH9Frame::H9FRAME_SOURCE_ID_MAX_VALUE) {
        PyErr_SetString(PyExc_OverflowError, ("maximum allowed value is " + std::to_string(ExtH9Frame::H9FRAME_SOURCE_ID_MAX_VALUE)).c_str());
        return -1;
    }
    ...
    if (data) {
        ...
        if (len > ExtH9Frame::H9FRAME_DATA_LENGTH) {
...
static PyObject* H9Frame_get_type(PyH9frame* self, void* closure) {
    return PyLong_FromUnsignedLong(self->frame.type);
}
...
static int H9Frame_set_type(PyH9frame* self, PyObject* value, void* closure) {
    ...
    if (tmp > ExtH9Frame::H9FRAME_TYPE_MAX_VALUE) {
        PyErr_SetString(PyExc_OverflowError, ("maximum allowed value is " + std::to_string(ExtH9Frame::H9FRAME_TYPE_MAX_VALUE)).c_str());
        return -1;
    }

    self->frame.type = (uint8_t)tmp;
    return 0;
}

static PyObject* H9Frame_get_seqnum(PyH9frame* self, void* closure) {
    return PyLong_FromUnsignedLong(self->frame.unicast.seqnum);
}
...
static int H9Frame_set_seqnum(PyH9frame* self, PyObject* value, void* closure) {
    ...
    if (tmp > ExtH9Frame::H9FRAME_SEQNUM_MAX_VALUE) {
        PyErr_SetString(PyExc_OverflowError, ("maximum allowed value is " + std::to_string(ExtH9Frame::H9FRAME_SEQNUM_MAX_VALUE)).c_str());
        return -1;
    }

    self->frame.unicast.seqnum = tmp;
    return 0;
}

static PyObject* H9Frame_get_destination_id(PyH9frame* self, void* closure) {
    return PyLong_FromUnsignedLong(self->frame.unicast.destination_id);
}
...
static int H9Frame_set_destination_id(PyH9frame* self, PyObject* value, void* closure) {
    ...
    if (tmp > ExtH9Frame::H9FRAME_DESTINATION_ID_MAX_VALUE) {
        PyErr_SetString(PyExc_OverflowError, ("maximum allowed value is " + std::to_string(ExtH9Frame::H9FRAME_DESTINATION_ID_MAX_VALUE)).c_str());
        return -1;
    }

    self->frame.unicast.destination_id = tmp;
    return 0;
}
...
static int H9Frame_set_source_id(PyH9frame* self, PyObject* value, void* closure) {
    ...
    if (tmp > ExtH9Frame::H9FRAME_SOURCE_ID_MAX_VALUE) {
        PyErr_SetString(PyExc_OverflowError, ("maximum allowed value is " + std::to_string(ExtH9Frame::H9FRAME_SOURCE_ID_MAX_VALUE)).c_str());
        return -1;
    }
...
PyMODINIT_FUNC PyInit_h9(void) {
    ...
    for (std::underlying_type_t<ExtH9Frame::Error> i = 0; i <= ExtH9Frame::H9FRAME_TYPE_MAX_VALUE; ++i) {
    ...
    for (std::underlying_type_t<ExtH9Frame::Type> i = 0; i <= ExtH9Frame::H9FRAME_TYPE_MAX_VALUE; ++i) {
