...
class Bus: public FrameSubject {
  private:
    ...
    std::atomic_bool run;
    std::map<int, BusEndpoint*> bus;

    IOEventQueue event_notificator;
    std::thread recv_thread_desc;

    std::queue<std::shared_ptr<BusFrame>> forward_queue;
    std::queue<std::shared_ptr<BusFrame>> send_queue;
    ...
    constexpr static int number_of_frame_types = 1 << H9FRAME_TYPE_BIT_LENGTH;
