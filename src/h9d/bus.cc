...
void Bus::add_driver(BusDriver* bus_driver) {
    auto* endpoint = new BusEndpoint(bus_driver);
    SPDLOG_LOGGER_INFO(logger, "Adding '{}' endpoint (driver: {})...", endpoint->name, endpoint->driver_name);
    bus[endpoint->open()] = endpoint;
}
